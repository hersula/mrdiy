...
class Format {
    ...
    public function factory($data, $from_type = NULL)
    {
        ...
        #return new static($data, $from_type); // Fungsi bawaan
		return $this->proses($data, $from_type); // Fungsi baru dibuat
    }
	
	public function proses($data = NULL, $from_type = NULL) // Fungsi baru dibuat
    {
        // Get the CodeIgniter reference
        $this->_CI = &get_instance();
        ...
        if ($from_type !== NULL)
        {
            if (method_exists($this, $from_type))
            {
                $data = call_user_func([$this, $from_type], $data);
            }
            else
            {
                throw new Exception('Format class does not support conversion from "'.$from_type.'".');
    ...
    public function to_xml($data = NULL, $structure = NULL, $basenode = 'xml')
    {
        ...
        foreach ($data as $key => $value)
        {
            ...
            {
                // make string key...
                $key = (singular($basenode) != $basenode) ? singular($basenode) : 'list';
            }
