...
// 			  }
	...
	function save($input) {
		...
		$data = array(
			...
			'nm_site' => $input['nm_site'],
			'status' => $input['status'],
			'created_by' => $this->session->userdata('user_id'),
			'creation_date' => date('Y-m-d H:i:s')
      );
		
		$NonQry = $this->db->insert('m_site', $data);
	...
	function update($input) {
		...
		$NonQry = $this->db->update('m_site', $data);
	...
	function getData2Edit($id) {
		$Qry = $this->db->query("SELECT a.id, a.kd_site, a.nm_site, a.status 
										FROM m_site a 
										WHERE a.id=?", array($id));
	...
	function getList($filter) {
		$this->datatables->select('a.id, a.kd_site, a.nm_site, a.status');
      $this->datatables->from('m_site a');
	...
	function getDataList($filter) {
		$this->db->select("a.kd_site, a.nm_site, CASE WHEN a.status=1 THEN 'Aktif' ELSE 'Tidak Aktif' END AS status");
      $this->db->from('m_site a');
		foreach($filter as $key => $val) {
			if (trim($val) != "" || !empty($val) || $val != NULL) {
   ...
   *******************************************************************************/
  	...
  	function getSiteList() {
		$this->datatables->select('a.kd_site, a.nm_site');
		$this->datatables->from('m_site a');
		return $this->datatables->generate();
	}

	function getSiteSelect2($input) {
		$Qry = $this->db->query("SELECT kd_site, nm_site FROM m_site WHERE kd_site ILIKE ? OR nm_site ILIKE ?", 
									   array('%'.$input['keyword'].'%', '%'.$input['keyword'].'%'));
		return $Qry->result();
	}

	function getSite() {
		$Qry = $this->db->query("SELECT a.id, a.kd_site, a.nm_site, a.status FROM m_site a");
		return $Qry->result();
	}
