...
class Progress_Model extends Core_Model {
...
// 			  }
	...
	function save($input) {
		...
		$data = array(
			...
			'nm_progres' => $input['nm_progres'],
			'status' => $input['status'],
			'created_by' => $this->session->userdata('user_id'),
			'creation_date' => date('Y-m-d H:i:s')
      );
		
		$NonQry = $this->db->insert('m_progres', $data);
	...
	function update($input) {
		...
		$NonQry = $this->db->update('m_progres', $data);
	...
	function delete($input) {
		...
		$NonQry = $this->db->delete("m_progres");
	...
	function getData2Edit($id) {
		$Qry = $this->db->query("SELECT a.id, a.kd_progres, a.nm_progres, a.status 
										FROM m_progres a 
										WHERE a.id=?", array($id));
	...
	function getList($filter) {
		$this->datatables->select('a.id, a.kd_progres, a.nm_progres, a.status');
      $this->datatables->from('m_progres a');
	...
	function getDataList($filter) {
		$this->db->select("a.kd_progres, a.nm_progres, CASE WHEN a.status=1 THEN 'Aktif' ELSE 'Tidak Aktif' END AS status");
      $this->db->from('m_progres a');
		foreach($filter as $key => $val) {
			if (trim($val) != "" || !empty($val) || $val != NULL) {
   ...
   *******************************************************************************/
  	...
  	function getProgresList() {
		$this->datatables->select('a.kd_progres, a.nm_progres');
		$this->datatables->from('m_progres a');
		return $this->datatables->generate();
	}

	function getProgresSelect2($input) {
		$Qry = $this->db->query("SELECT kd_progres, nm_progres FROM m_progres WHERE kd_progres ILIKE ? OR nm_store ILIKE ?", 
									   array('%'.$input['keyword'].'%', '%'.$input['keyword'].'%'));
		return $Qry->result();
	}

	function getProgres() {
		$Qry = $this->db->query("SELECT a.id, a.kd_progres, a.nm_progres, a.status FROM m_progres a");
		return $Qry->result();
	}
