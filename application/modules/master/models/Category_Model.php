...
// 			  }
	...
	function save($input) {
		...
		$data = array(
			...
			'nm_category' => $input['nm_category'],
			'status' => $input['status'],
			'created_by' => $this->session->userdata('user_id'),
			'creation_date' => date('Y-m-d H:i:s')
      );
		
		$NonQry = $this->db->insert('m_category', $data);
	...
	function update($input) {
		...
		$NonQry = $this->db->update('m_category', $data);
	...
	function delete($input) {
		...
		$NonQry = $this->db->delete("m_category");
	...
	function getData2Edit($id) {
		$Qry = $this->db->query("SELECT a.id, a.kd_category, a.nm_category, a.status 
										FROM m_category a 
										WHERE a.id=?", array($id));
	...
	function getList($filter) {
		$this->datatables->select('a.id, a.kd_category, a.nm_category, a.status');
      $this->datatables->from('m_category a');
	...
	function getDataList($filter) {
		$this->db->select("a.kd_category, a.nm_category, CASE WHEN a.status=1 THEN 'Aktif' ELSE 'Tidak Aktif' END AS status");
      $this->db->from('m_category a');
		foreach($filter as $key => $val) {
			if (trim($val) != "" || !empty($val) || $val != NULL) {
   ...
   *******************************************************************************/
  	...
  	function getCategoryList() {
		$this->datatables->select('a.kd_category, a.nm_category');
		$this->datatables->from('m_category a');
		return $this->datatables->generate();
	}

	function getCategorySelect2($input) {
		$Qry = $this->db->query("SELECT kd_category, nm_category FROM m_category WHERE kd_category ILIKE ? OR nm_category ILIKE ?", 
									   array('%'.$input['keyword'].'%', '%'.$input['keyword'].'%'));
		return $Qry->result();
	}

	function getCategory() {
		$Qry = $this->db->query("SELECT a.id, a.kd_category, a.nm_category, a.status FROM m_category a");
		return $Qry->result();
	}
