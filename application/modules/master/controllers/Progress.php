...
class Progress extends Core_Controller {
	...
	function __construct(){
		parent::__construct("progress"); # parsing menu_id
		$this->load->model("Progress_Model", "m_app");
	}
   ...
   *******************************************************************************/
	...
	function pdf() {
		...
		if ($data->result() != NULL) {
			foreach($data->result() as $key => $val) {
				$pdf->Cell(50,5,$val->kd_progres,1,0,'L');
			  	$pdf->Cell(80,5,$val->nm_progres,1,0,'L');
	...
	function xls() {
		...
		$Col['header'] = array('Status Code', 'Status Name',  'Status');
