...
class Type extends Core_Controller {
	...
	function __construct(){
		parent::__construct("type"); # parsing menu_id
		$this->load->model("Type_Model", "m_app");
	}
   ...
   *******************************************************************************/
	...
	function pdf() {
		...
		if ($data->result() != NULL) {
			foreach($data->result() as $key => $val) {
				$pdf->Cell(50,5,$val->kd_type,1,0,'L');
			  	$pdf->Cell(80,5,$val->nm_type,1,0,'L');
	...
	function xls() {
		...
		$Col['header'] = array('Type Code', 'Type Name',  'Status');
