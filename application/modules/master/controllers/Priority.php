...
class Priority extends Core_Controller {
	
	function __construct(){
		parent::__construct("priority"); # parsing menu_id
		$this->load->model("Priority_Model", "m_app");
	}
   ...
   *******************************************************************************/
	...
	function index() {
		...
		$this->load->view('Priority', $this->data);
	}
	...
	function pdf() {
		...
		if ($data->result() != NULL) {
			foreach($data->result() as $key => $val) {
				$pdf->Cell(50,5,$val->kd_priority,1,0,'L');
			  	$pdf->Cell(80,5,$val->nm_priority,1,0,'L');
	...
	function xls() {
		...
		$Col['header'] = array('Priority Code', 'Priority Name',  'Status');
