...
class Category extends Core_Controller {
	...
	function __construct(){
		parent::__construct("category"); # parsing menu_id
		$this->load->model("Category_Model", "m_app");
	}
   ...
   *******************************************************************************/
	...
	function pdf() {
		...
		if ($data->result() != NULL) {
			foreach($data->result() as $key => $val) {
				$pdf->Cell(50,5,$val->kd_category,1,0,'L');
			  	$pdf->Cell(80,5,$val->nm_category,1,0,'L');
	...
	function xls() {
		...
		$Col['header'] = array('Category Code', 'Category Name',  'Status');
