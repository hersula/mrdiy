...
class Site extends Core_Controller {
	...
	function __construct(){
		parent::__construct("site"); # parsing menu_id
		$this->load->model("Site_Model", "m_app");
	}
   ...
   *******************************************************************************/
	...
	function pdf() {
		...
		if ($data->result() != NULL) {
			foreach($data->result() as $key => $val) {
				$pdf->Cell(50,5,$val->kd_site,1,0,'L');
			  	$pdf->Cell(80,5,$val->nm_site,1,0,'L');
	...
	function xls() {
		...
		$Col['header'] = array('Site Code', 'Site Name',  'Status');
