...
function initPage(){
	...
	DataTable = $('#table_list_data').DataTable({ 
		...
		"columns": [
			...
			{"data": "go_date", "className": "text-center"},
			{"data": "employee_name"},
		...
		"columnDefs": [
			...
			},
			{
				...
				"searchable": false,
				"render": function ( data, type, row ) {
					return moment(row.go_date).format('DD/MM/YYYY');
				},
			},
			{
				"targets": 8,
...
function LOVFilterReg() {
	$('#PopUpModal').load(base_url + 'setting/regional/getRegionalList/', () => { // Ambil URL untuk membuka modal LOV
