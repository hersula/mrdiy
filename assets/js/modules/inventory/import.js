...

			{
				...
				"render": function ( data, type, row ) {
					let actions = '';
						...
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.no_spbu + '\');"><i class="fas fa-trash"></i></button>';
					return actions;
				},
			},
		],
...
function upload_data() {
	...
	MsgBox.Confirm('Yakin akan Upload data ini?').then(result => {
		let url =  base_url + Modules + '/import/upload_data/';
		$.ajax({
			url: url,
			type: 'POST',
			dataType: 'json',
...
function download_data() {
    window.location.href = base_url + 'assets/data_upload_sku/format_upload_sku.xlsx';
