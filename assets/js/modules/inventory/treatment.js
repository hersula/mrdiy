			...
			},
			{
				...
				"render": function ( data, type, row ) {
					...
					if (Priv.delete_flag == 1) {
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.treat_name + '\');"><i class="glyphicon glyphicon-trash"></i> </button>';
					}
					return actions;
				},
			},
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#treat_id').val(row.treat_id); $('#treat_id').prop('readonly', true);
			$('#treat_name').val(row.treat_name);
...
function app_delete(multi = 0, id = null, menu_id = '') {
	if (multi) {
		if (dataArr.length <= 0) {
			MsgBox.Notification('Checklist terlebih dahulu data yang akan dihapus.', 'bottom right', 'treatment');
			return;
		}
		data2Send = JSON.stringify({
			id: dataArr
