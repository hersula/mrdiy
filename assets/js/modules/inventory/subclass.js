...
function initPage(){
	...
	DataTable = $('#table_list_data').DataTable({ 
		...
		"columns": [
			{"data": "subclass_id", "width": 80, "className": "text-left"},
			{"data": "subclass_name"},
			{"data": "mclass_id"},
			...
			},
			{
				...
				"render": function ( data, type, row ) {
					...
					if (Priv.delete_flag == 1) {
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.submclass_name + '\');"><i class="glyphicon glyphicon-trash"></i> </button>';
					}
					return actions;
				},
			},
	...
	MsgBox.Confirm('Yakin akan simpan data ini?', 'Simpan data').then(result => {
		...
		.then((data) => {
			if (data.result) {
				...
				data2Send = null;
				idData = null;
				$('#subclass_id').prop('readonly', true);
			} else {
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#subclass_id').val(row.subclass_id); $('#subclass_id').prop('readonly', true);
			$('#subclass_name').val(row.subclass_name);
			$('#mclass_id').val(row.mclass_id);
...
function app_delete(multi = 0, id = null, menu_id = '') {
	if (multi) {
		if (dataArr.length <= 0) {
			MsgBox.Notification('Checklist terlebih dahulu data yang akan dihapus.', 'bottom right', 'subclass');
			return;
		}
		data2Send = JSON.stringify({
			id: dataArr
