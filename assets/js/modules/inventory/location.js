			...
			},
			{
				...
				"render": function (data, type, row) {
					...
					if (Priv.delete_flag == 1) {
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.loc_name + '\');"><i class="glyphicon glyphicon-trash"></i> </button>';
					}
					return actions;
				},
			},
...
function SearchStore() {
	$('#PopUpModal').load(base_url + 'inventory/location/getStoreList/', () => { // Ambil URL untuk membuka modal LOV
...
function SearchArea() {
	$('#PopUpModal').load(base_url + 'inventory/location/getAreaList/', () => { // Ambil URL untuk membuka modal LOV
		$('#ModalLOV').modal('show'); // Tampilkan modal LOV
		$(".modal-dialog").css({ width: "600px" }); // Lebar modal LOV
