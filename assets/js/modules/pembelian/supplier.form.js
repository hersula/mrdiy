(function () {
	...
	// edit action
		...
		FetchWithTimeout(base_url + Modules + '/' + Controller + '/getData2Edit/json/' + id, null, 'GET', 7000)
		.then((data) => {
			if (data.result) {
				var row = data.data;

				$('.select2-usaha').select2().val(row.typeusaha_id).trigger("change");
				// $('#type_usaha').val(row.typeusaha_id); 
				...
				$('#disclaimer').val(row.status_disclaimer);	
...
function Simpan() {
	let list_input = ['supp_name', 'alias','pic','jabatan','no_hp','no_ktp','bank_id_1','kantor_cabang1','city_name1','no_rek1','nama_rek1','cur_id_1'];
	...
	formData.append('supp_id', data2Send.supp_id);
	formData.append('supp_name', data2Send.supp_name);	
	formData.append('alias', data2Send.alias);
	formData.append('holding_id', data2Send.holding_id);
	...
	formData.append('disclaimer', data2Send.disclaimer);
	...
	MsgBox.Confirm('Yakin akan simpan data ini?').then(result => {
		...
		// FetchWithTimeout(url, formData, 'POST', 5000)
		// 	.then((data) => {
		// 		if (data.result) {
...
function notActiveBrand(id, brand, active) {
	MsgBox.Confirm('Are You Sure Not Active Brand ' + brand  + ' ?').then(result => {
		SendAjax(base_url + Modules + '/' + Controller + '/notActiveBrand/json/' + id + '/' + $('#supp_id').val() + '/' + active, null, 'GET', 'JSON', 5000, successNotActiveBrand);
	}).catch(err => {
		console.log(err);
	});
}
...
function LOVHolding() {
	$('#PopUpModal').load(base_url + 'pembelian/supplier/getHoldingList/home/', () => { // Ambil URL untuk membuka modal LOV
		$('#ModalLOV').modal('show'); // Tampilkan modal LOV
		$('.modal-dialog').attr('class','modal-dialog modal-lg'); // Ukuran modal LOV
		$('#list_cols').val(['holding_id', 'holding_name']);
...
function SaveBrand() {
	let RowIdx = $(".id_row").val();
	let getData = DatatableBrand.row(RowIdx).data();
	let url = base_url + Modules + '/' + Controller + '/approvebrand/';

	let dat_object = {
		supps_id : $(".supp_id").val(),
		ket_brand : $(".ket_brand").val(),
		brand_code : $(".brand_code").val(),
	}
	// console.log(dat_array);

	MsgBox.Confirm('Yakin Kode Brand Sudah benar ?').then(result => {
		SendAjax(url, dat_object, 'POST', 'JSON', 5000, successSaveBrand);
		DeleteRowDataTable(DatatableBrand, RowIdx);
	}).catch(err => {
		console.log(err);
	});
	$("#viewBrandCode").modal('hide');
}
function successSaveBrand(data) {
	let update_table = DatatableBrand.rows().data().toArray();
	let url = base_url + Modules + '/' + Controller + '/updatebrand/';
	let dat_object = {
		supp_id : supp_id,
		ket_brand : update_table
	}
	SendAjax(url, dat_object, 'POST', 'JSON', 5000, successUpdateBrand);
}
function successUpdateBrand(data) {
	if (data.result == true) {
		MsgBox.Notification(data.msg.toString(), 'bottom right', 'success');
		data2Send = null;
		if (action === 'create') {
			window.location.href = '#' + Modules + '/' + Controller + '/edit/' + supp_id;
		} else {
			Kembali();
		}
	}else{
...
function LoadDataTable() {	
	...
	DataTable = $('#table_list_data').DataTable({
		...
		"responsive": true,
		"ajax": {
			"url": base_url + Modules + '/' + Controller + '/getListbrand/' + id,
		...
		"columnDefs": [
			{
				...
				"render": function (data, type, row) {
					if (row.status == 1) {
						...
						return  '<span class="badge badge-danger text-center">Tidak Aktif</span>   <button class="btn btn-outline-success waves-effect waves-light btn-sm text-center" onclick="notActiveBrand(\'' + row.brand_id + '\',\'' + row.brand_name + '\',\'' + '1' + '\')">Active Brand</button>';
					}
				},
			},
			{
