			...
			},
			{
				...
				"render": function ( data, type, row ) {
					...
					if (Priv.delete_flag == 1) {
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.wil_name + '\');"><i class="glyphicon glyphicon-trash"></i> </button>';
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#bank_id').val(row.bank_id); $('#bank_id').prop('readonly', true);
...
function app_delete(multi = 0, id = null, menu_id = '') {
	if (multi) {
		...
		MsgBox.Confirm('Hapus Data Bank ini ?').then(result => {
		...
		MsgBox.Confirm('Hapus Bank ' + menu_id + ' ?').then(result => {
			FetchWithTimeout(base_url + Modules + '/' + Controller + '/delete/', data2Send, 'POST', 5000)
			.then((data) => {
				if (data.result) {
					MsgBox.Notification(data.msg.toString(), 'bottom right', 'success');
