...
function Simpan() {
	...
	data2Send = JSON.stringify({
		id: idData,
		level_code: $('#aspek_code').val(),
		level_name: $('#aspek_name').val(),
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#aspek_code').val(row.aspek_code); $('#aspek_code').prop('readonly', true);
			$('#aspek_name').val(row.aspek_name);
...
function ResetForm() {
	action = 'create';
	$('#aspek_code').prop('readonly', false);
