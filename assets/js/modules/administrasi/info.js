			...
			},
			{
				...
				"render": function ( data, type, row ) {
					...
					if (Priv.delete_flag == 1) {
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.info_name + '\');"><i class="glyphicon glyphicon-trash"></i> </button>';
					}
					return actions;
				},
			},
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#info_group').val(row.info_group);
			$('#info_name').val(row.info_name);
