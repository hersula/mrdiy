			...
			},
			{
				...
				"render": function ( data, type, row ) {
					...
					if (Priv.delete_flag == 1) {
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.kd_site + '\');"><i class="fas fa-trash"></i></button>';
					}
					return actions;
				},
			},
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#kd_site').val(row.kd_site); $('#kd_site').prop('readonly', true);
			$('#nm_site').val(row.nm_site);
