...
function initPage(){
	...
	DataTable = $('#table_list_data').DataTable({ 
		...
		"columns": [
			{"data": "store_name", "width": 80, "className": "text-left"},
			{"data": "booth_id", "width": 80, "className": "text-left"},
			{"data": "booth_name"},
			{"data": "loc_id"},
			{"data": "luas", "width": 40, "className": "text-right"},
			...
			},
			{
				...
				"render": function ( data, type, row ) {
					...
					if (Priv.delete_flag == 1) {
						actions = actions + '<button type="button" class="btn btn-outline-danger btn-sm waves-effect waves-light" onclick="app_delete(0, ' + row.id + ', \'' + row.booth_name + '\');"><i class="glyphicon glyphicon-trash"></i> </button>';
					}
					return actions;
				},
			},
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#store_name').val(row.store_name); $('#store_name').prop('readonly', true);
			$('#booth_id').val(row.booth_id); $('#booth_id').prop('readonly', true);
			$('#booth_name').val(row.booth_name);
...
function LOVLoc() {
	$('#PopUpModal').load(base_url + 'spacemanagement/lahansewa/getLocList/', () => { // Ambil URL untuk membuka modal LOV
		$('#ModalLOV').modal('show'); // Tampilkan modal LOV
		$(".modal-dialog").css({width: "600px"}); // Lebar modal LOV
		$('#list_cols').val(['loc_id', 'loc_name']);
		$('#list_controls').val(['#loc_id', '#loc_name']);
	});
}

function LOVLocClear() {
	$('#loc_id').val('');
	$('#loc_name').val('');
}
