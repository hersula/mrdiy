...
function Simpan() {
	...
	data2Send = JSON.stringify({
		...
		price_grade_name: $('#price_grade_name').val(),
		margin: accounting.unformat($('#margin').val()),
...
function app_edit(id = null) {
	...
	.then((data) => {
		if (data.result) {
			...
			let row = data.data;
			idData = row.id
			$('#price_grade_code').val(row.price_grade_code); $('#price_grade_code').prop('readonly', true);
			$('#price_grade_name').val(row.price_grade_name);
			$('#margin').val(accounting.formatNumber(row.margin, 2));
...
function ResetForm() {
	action = 'create';
	$('#price_grade_code').prop('readonly', false);
